...
const ChartsWidget1: React.FC<Props> = ({className,data,date,region,users}:Props) => {
  ...
  }, [chartRef, mode,data,date,region,users])
  ...
  const refreshChart = () => {
    ...
    const chart = new ApexCharts(chartRef.current, getChartOptions(height,data,date,region,users))
    if (chart) {
      chart.render()
    }
...
function getChartOptions(height: number,data:DataInterface[],date:TDate[],region:Region[],users:Users[]): ApexOptions {
  ...
  return {
    series: [
      ...
      {
        name: 'Users all over',
        // data: data.filter(d=>d.region==='Gujarat').map(d=>Number(d.users))
