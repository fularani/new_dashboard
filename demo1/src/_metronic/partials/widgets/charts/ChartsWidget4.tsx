...
import {DataInterface,TDate,Users
  // ,Region,Country,City
...
const ChartsWidget4: React.FC<Props> = ({className,date,data,users}:Props) => {
  ...
  const refreshChart = () => {
    ...
    const chart = new ApexCharts(chartRef.current, getChartOptions(height,date,data,users))
    if (chart) {
      chart.render()
    }
...
function getChartOptions(height: number,date:TDate[],data:DataInterface[],users:Users[]): ApexOptions {
